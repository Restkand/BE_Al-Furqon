"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.deleteBeacon = exports.updateBeacon = exports.insertBeacon = exports.getListBeacon = void 0;
const prisma_1 = __importDefault(require("./prisma"));
const getListBeacon = async () => {
    try {
        return await prisma_1.default.ms_beacon.findMany({
            where: { status: "A" },
            select: {
                id: true,
                beacon_id: true,
                beacon_name: true,
                beacon_battery: true,
                beacon_mac_addr: true,
                beacon_battery_at: true,
                status: true,
            },
        });
    }
    catch (error) {
        console.error("Failed to get list vendor:", error);
        throw error;
    }
};
exports.getListBeacon = getListBeacon;
const insertBeacon = async (beaconCode, site, name, beaconMacAddr, beaconBattery, BeaconBatteryCreat, created_by, updated_by) => {
    try {
        return await prisma_1.default.ms_beacon.create({
            data: {
                beacon_id: beaconCode,
                siteid: site,
                beacon_name: name,
                beacon_mac_addr: beaconMacAddr,
                beacon_battery: beaconBattery,
                beacon_battery_at: BeaconBatteryCreat,
                status: "A",
                created_by: created_by,
                created_at: new Date(),
                updated_by: updated_by,
                updated_at: new Date(),
            },
        });
        //    const result = await prisma.$executeRaw`
        //     INSERT INTO "ms_beacon" (beacon_id,siteid,beacon_name,employee_id,beacon_mac_addr,beacon_battery,beacon_battery_at,status,created_by,created_at,updated_by,updated_at)
        //     VALUES (${beaconCode},${site},${name},${employee},${beaconMacAddr},${beaconBattery},${BeaconBatteryCreat},${status},${created_by},NOW(),${updated_by},NOW())
        //     `
        //     return result;
    }
    catch (error) {
        console.error("Failed to create beacon:", error);
        throw error;
    }
};
exports.insertBeacon = insertBeacon;
const updateBeacon = async (site, name, beaconMacAddr, beaconBattery, BeaconBatteryCreat, status, updated_by, id) => {
    try {
        const idAsNumber = Number(id);
        return await prisma_1.default.ms_beacon.update({
            where: { id: idAsNumber },
            data: {
                siteid: site,
                beacon_name: name,
                beacon_mac_addr: beaconMacAddr,
                beacon_battery: beaconBattery,
                beacon_battery_at: BeaconBatteryCreat,
                status: status,
                updated_by: updated_by,
                updated_at: new Date(),
            },
        });
        // const result = await prisma.$executeRaw`
        // update "ms_beacon"
        // SET
        // siteid =${site},
        // beacon_name = ${name},
        // employee_id = ${employee},
        // beacon_mac_addr= ${beaconMacAddr},
        // beacon_battery = ${beaconBattery},
        // beacon_battery_at = ${BeaconBatteryCreat},
        // status = ${status},
        // updated_by = ${updated_by},
        // updated_at = NOW()
        // WHERE id = ${idAsNumber}
        // `
        // return result
    }
    catch (error) {
        console.error("Failed to create beacon:", error);
        throw error;
    }
};
exports.updateBeacon = updateBeacon;
const deleteBeacon = async (updated_by, id) => {
    try {
        const idAsNumber = Number(id);
        return await prisma_1.default.ms_beacon.update({
            where: {
                id: idAsNumber,
            },
            data: {
                status: "X",
                updated_at: new Date(),
                updated_by: updated_by,
            },
        });
    }
    catch (error) {
        console.error("Failed to create beacon:", error);
        throw error;
    }
};
exports.deleteBeacon = deleteBeacon;
