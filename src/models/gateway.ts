...
export const listGatewayByFloor = async (floorid: string, site: string) => {
  try {
    const result = await prisma.$queryRaw`
        ...
        x_percent,y_percent,
        status 
        from ms_gateway where floor_id = ${String(
          floorid
        )} and siteid = ${site} and status = 'A'
        `;
    return result;
  } catch (error) {
    console.error("Failed to get list vendor:", error);
...
export const insertGateway = async (
  gateway_id: string,
  site: string,
  name: string,
  floor: string,
...
export const getListGatewayByRoute = async (
  routeid: any
): Promise<JoinedGatewayRoute[]> => {
  try {
